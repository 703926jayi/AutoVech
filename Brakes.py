...
try:
    ...
    HAS_HARDWARE = False
    print("Running in SIMULATION mode: Physical GPIO not detected.")
import spidev
import rclpy
...
class BrakesNode(Node):
    ...
    def read_adc(self):
        ...
        if not HAS_HARDWARE:
            return 512  # Return a middle-ground dummy value for simulation
    ...
    def move(self, direction, speed_pct=100):
        ...
        if direction == 'extend':
            if HAS_HARDWARE:
                ...
                self.get_logger().info("Simulating: Extending Brakes")
            ...
            if HAS_HARDWARE:
                ...
                self.get_logger().info("Simulating: Retracting Brakes")
                
        else:
            # SAFETY FALLBACK: If direction is unknown, stop everything.
            self.stop()
            return

        # Apply speed (PWM) only if hardware exists
        if HAS_HARDWARE:
